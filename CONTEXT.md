...
**AuthContext**:
A specific authenticated state belonging to a Principal — a bearer token, session cookie, or API key — with a validity window and observed capabilities. Token rotation mints a **new** AuthContext; what stays stable across rotation is the Principal's **credential slot** (see below).
...
The dispatch analog of a **coverage gap** (ADR-0053): a `TestCase` whose only `primary` `EXECUTED_AS` edges are `auth_invalid` *and* whose attacker slot has since rotated past the failure (see **rotation watermark**). Computed at query time over existing axes (`dispatch_status`, edge `at`, AuthContext `first_seen`/`status`) — **never a stored flag**, the same pull/ephemeral discipline as coverage. Surfaced for manual rerun (selected by `key_hash`); auto re-dispatch is a later staging-only layer, since production needs a fresh mode-gated gate at dispatch time (ADR-0042). A candidate below its watermark is "waiting on rotation" and is refused even on manual rerun.
_Avoid_: parked, retry (a stored lifecycle state — there is none).

**rotation watermark**:
The `first_seen` of a slot's newest `active` declared `AuthContext` generation (ADR-0053). An `auth_invalid` `primary` edge is re-dispatch-eligible only when a newer-than-`edge.at` active generation exists — i.e. the credential precondition that caused the failure has *demonstrably* cleared. This makes `auth_invalid` transient-**but-gated**: re-dispatchable because the precondition changed, not merely because the status is "transient". Below the watermark, re-dispatch is refused (it would re-send against the same dead slot and storm).
_Avoid_: timestamp, cursor.

**unverified / verified rotation entry**:
A freshly-minted slot credential the auth-helper has written to the rotation file but the Dispatcher has not yet liveness-confirmed (ADR-0053). The helper stays **target-blind** (it only calls the IdP), so it publishes `unverified`; on first use the Dispatcher runs the ADR-0044 probe *before* the `primary` — live → promote to `verified` and proceed, dead → refuse the `primary` (don't burn it), emit the reactive event, and record a **failed mint** (after K failed mints on a slot the helper backs off with `slot unrecoverable` rather than storming). Resolution is therefore no longer purely positional (ADR-0049): an `unverified` entry is provisional until promoted.
_Avoid_: stale (an unverified entry may well be live — it is simply unproven).
...
- **TestCase identity** is content-addressed: `key_hash = sha256(canonicalized(engagement_id, test_class, target_endpoint_id?, target_parameter_id?, target_trust_boundary_id?, payload_class, payload_hash, attacker_principal, attacker_slot))`, unique-indexed. The target is a **three-way XOR**: exactly one of `target_endpoint_id` (route-level test), `target_parameter_id` (parameter-level test; the Parameter node carries its Endpoint via `HAS_PARAMETER`), or `target_trust_boundary_id` (boundary test). The other two normalize to null and fall out of the hash. `payload_hash` is over the concrete bytes the dispatcher will send (sentinel `sha256("")` for no-payload tests; never SQL null). The attacker is keyed by **`(attacker_principal, attacker_slot)`** — the rotation-stable credential slot (ADR-0049), not the content-addressed `auth_context_id`, so token rotation does not mint a duplicate TestCase; `auth_context_id` rides as a **non-key property** updated `ON MATCH` — it is the **armable** AC the planner resolved for that `(attacker_principal, attacker_slot)` (the dispatch-side slot-resolution handle), not necessarily the AC observed in the evidencing traffic: when the endpoint was hit only under a discovered-tier AC of a declared Principal, the assembler substitutes the Principal's declared AC (#133). Anonymous tests use `("anonymous", "anonymous")`. Engagement is part of the identity, so cross-Engagement re-runs are distinct nodes.

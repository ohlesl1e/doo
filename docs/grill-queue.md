...
| Planner LLM provider | Claude (`claude-opus-4-8`) via litellm | Planner is the highest-leverage reasoning task; code against one client so provider is config, not code. **Routing is litellm prefix-based** (`anthropic/…` vs `openai/…`); per-engagement, per-role *model* defaults persist on the `Engagement` node (ADR-0051). The earlier per-engagement `llm.provider: gateway\|local` data-policy enum was **dropped** — speculative, never wired. |
...
- **Injection-class generators (XSS / SQLi / CMDi) + coverage shape.** The `sink` generator is deliberately scoped to URL/path-shaped params (`SinkRole = redirect_target | url_sink | file_path` → SSRF / open-redirect / path-traversal); reflection / query / shell sinks are different detection heuristics + probe payloads + interpreter signals, so they want their own generator(s). Shape question to grill: one `inject` generator with a `SinkRole`-style sub-role enum (mirrors `sink`; one `-g` id, role discriminates the constructor) vs. per-class ids (`xss` / `sqli` / `cmdi`; finer `-g` control, more `GeneratorId` sprawl). Either way it depends on the **payload-synthesis library** above (these classes need fuzz lists, not a single `configured` probe) and the per-environment OPA `payload_class` rules. **Coverage:** no new `cN` — black-box can't know a parameter is a SQL/shell/HTML sink pre-probe, so "params with no injection TestCase" degenerates to "every param". The C5 family already answers "what's untested" once these generators propose; per-class visibility is a `--class <test_class>` filter on C5/C5a/C5b, not a new top-level query. A "detected-but-unproposed sink params" view is just the generator's candidate selector diffed against committed TestCases — closer to `planner propose --dry-run -g <id>` than coverage. Surfaced 2026-06-18.
...
- ~~**Per-engagement LLM provider/model wiring.**~~ ✅ Grilled 2026-06-22 → **ADR-0051**. `provider` dropped (the data-policy rationale was speculative; routing is litellm prefix + operator infra, not doo's concern). `LLMConfig = {model, interpreter_model}`, persisted on the `Engagement` node at `engagement start`; `--model` flag on `planner propose` / `dispatch run`; precedence CLI > env > Engagement node > default. `DOO_LLM_API_BASE` (+ per-role `DOO_PLANNER_API_BASE` / `DOO_INTERPRETER_API_BASE`) are env-only force-pins whose normal state is *unset* — multi-provider works via litellm's own `ANTHROPIC_API_KEY` / `OPENAI_API_BASE` / etc. Surfaced 2026-06-13 (#95).
- **`dispatch run` / `auth-helper run` drop `--config` and become id-only.** Surfaced during the ADR-0051 grill (2026-06-22). `planner propose` already works from `-e <id>` + graph alone; dispatch re-reads the YAML each run for (a) credential `${VAR}` env-ref *names* → `EnvSecretStore`, (b) `cfg.dispatch.*` operational tuning (tls_verify, liveness regexes, rate caps), and (c) `cfg.environment` — which is *also* on the `Engagement` node already, so (c) is plain redundancy. Grill: persist (a) as an `env_var_ref` property on the declared `AuthContext` node (the var *name* is not secret; ADR-0012 amendment is one line) and (b) on the `Engagement` node, both via the ADR-0019 idempotent loader; then `EnvSecretStore` builds from `build_declared_slot_map` instead of `cfg`. Open sub-question: is `dispatch.rate_limit` an engagement fact (persist) or a per-invocation knob (keep as flag/env)? Either way the secret *value* still resolves from `os.environ` at send time — only the lookup *key* moves into the graph.
...
6. ~~**Slice 3 — LLM-assisted hypothesis generation (the Planner).**~~ ✅ grilled 2026-06-08 (ADR-0036–0041, incl. a second-order review pass amending 0036/0037/0040). Decisions:
   ...
   - **LLM provider** — Claude via litellm; per-role model selection + precedence per **ADR-0051** (the slice-3 `provider: gateway|local` enum was dropped as speculative).

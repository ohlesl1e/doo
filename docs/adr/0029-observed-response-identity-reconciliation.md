...
- **Unify with ADR-0027's `discovered:jwt:*` namespace** (key purely on `(type, value)` regardless of where the identity was seen) — deferred: more correct long-term (one Principal per real user across all evidence) but re-keys existing JWT identities and widens merge surface. This slice keeps a separate `discovered:observed:*` namespace; cross-signal unification is a later, deliberate step.
...
- **Gap (accepted):** a user identified by a JWT claim and (elsewhere) by an observed header with the same value stays as two Principals across the two namespaces until cross-signal unification lands.

...
| C3 | Leak-to-input pivots (issue #53): an `ObservedValue` that is BOTH a response output (`YIELDED_VALUE`) and a request input (`SENT_VALUE`) — a concrete "what to test next" lead. INDEPENDENT of the `reached` predicate. Cross-endpoint by default (source ≠ target); same-endpoint reuse behind `--include-same-endpoint`. The **target** (input) endpoint must pass `is_in_scope` (ADR-0020); the **source** (output) endpoint need NOT (a value leaked from an out-of-scope SSO host is still a valid lead). Temporality ignored; only promoted values are nodes so junk is excluded (ADR-0009); secret-shaped values surface `value_hash` + preview only, never raw (ADR-0015); engagement-scoped + `status='active'` (ADR-0017); ranked by value-shape specificity (UUID/email/JWT > opaque_token > bare integer) then decayed confidence (ADR-0005), `--min-confidence` opt-in. Principal/tenant-aware C3 is OUT of scope (deferred). | `tests/test_coverage_c3_unit.py` (cross-endpoint included, same-endpoint excluded-by-default/included-with-flag, mixed-sources keep cross-only, target-out-of-scope excluded, source-out-of-scope still surfaces, secret hash+preview-only, shape-rank ordering, within-rank confidence ordering, decay + `--min-confidence`, JSON round-trip); `tests/test_coverage_c3_e2e.py::test_c3_surfaces_cross_endpoint_pivot` (golden e2e: `/widgets` response UUID → `/detail/item/page` `widget_id` input surfaces; same-endpoint `/sync/cursor` reuse excluded by default, appears with `--include-same-endpoint`; out-of-scope-target `other.test` pivot excluded); `tests/test_coverage_cli.py` (c3 table + `--json` + `--include-same-endpoint` pass-through rendering) |

...
### Slice 4 — Bounded agent execution ✅ shipped

The first slice that **sends traffic**. A `doo dispatch run` is the *authorization unit* (ADR-0042): one human arming decision drains a budget-bounded `DispatchSelection` of `approved` `TestCase`s. Every wire send passes one **Dispatcher** gate — kill-switch lease (read-only, ADR-0014) → wallclock + request budget → **real OPA** (`opa eval` over a `Scope`-generated bundle + a fixed Rego ruleset, ADR-0046; the planner's `is_in_scope` is *not* a substitute) → wire — with no side channel. The **Executor** is deterministic, narrow constructors (`(test_class, role)` → `ConcreteRequest`, ADR-0043); the LLM never composes request bytes. The **Interpreter** runs a bounded multi-turn confirm loop (ADR-0045): it picks `RequestRole`s from a closed per-`test_class` enum (`primary` / `baseline_victim` / `baseline_negative`), reads bodies on demand, and emits a typed `InterpreterVerdict` — the **fourth orthogonal `TestCase` axis** (`status` / `review_status` / `dispatch_status` / `interpreter_verdict`). A `vulnerable` verdict commits a `Finding@proposed` (ADR-0045); `doo finding review` is its human gate.

The four `TestCase` axes and the request roles are closed enums; the Interpreter's only wire authority is *which role next*. Key mechanisms, by tracer (ADR-0042–0047):

- **`dispatch_status`** (ADR-0013/0044): an authz `primary` 4xx is disambiguated by a per-`AuthContext` **liveness probe** (`auth_invalid` if the token is dead → an ADR-0014 reactive-refresh event; `replay_invalid` if a hazard is unverified; else `ok`), with an optional body-match short-circuit. Cached per `(AuthContext, window)`.
- **Replay-hazard resolution** (ADR-0041/0043): CSRF tokens are warm-fetched + spliced, nonces stripped, timestamps set; an unresolved hazard refuses the send (`hazard_unresolved`) and surfaces in **`doo dispatch review`** (`--set-hint` / `--ignore-hazard`).
- **Auth-helper sibling** (ADR-0014): `doo auth-helper run` rotates declared tokens proactively + reactively (consuming the `auth_invalid` stream), rate-limited; the agent only reads rotated material via a file overlay.
- **Coverage**: `EXECUTED_AS{dispatch_status, request_role, run_id}` edges make **C5/C5a/C5b** (`TrustBoundary`s not proposed / approved / executed-to-verdict, ADR-0047) meaningful.
- **Follow-ups + modes** (ADR-0042/0045): the Interpreter's `follow_ups` go back through the slice-3 Validator → `proposed` (`source=llm-interpreter`); the `InterpreterMode` strategy ships `confirm` only (`freelance` is a staging-only seam that raises).

CLI surface: `doo dispatch run` / `doo dispatch review` / `doo finding review` / `doo auth-helper run`, alongside `doo engagement keepalive` (the kill switch). Tester identity stays out of the graph — the dispatch ledger holds it (ADR-0040).

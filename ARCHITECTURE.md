...
The first slice with an LLM. **Planner** (LLM proposes structured `{test_class, target_node_id, parameters, justification}` — never request strings) → **Validator** (deterministic: planner-side OPA, graph consistency, dedup). Human approves; **nothing is dispatched yet.** Pulls in the remaining ontology piece — **`TrustBoundary` inference** — so authz boundaries become first-class, test-targetable nodes, which unblocks **C4** (capability-tier transitions never exercised) as a coverage query. Creates `TestCase` nodes (proposed, content-addressed per ADR-0007; no `EXECUTED_AS` edge yet, gated by a `review_status` human-review lifecycle per ADR-0040). Grilled 2026-06-08 (ADR-0036–0040): gap-driven planner with pluggable deterministic candidate generators (ADR-0036); a typed context-pack→`PlannerProposal` I/O contract (ADR-0037); planner-side policy via the shared `is_in_scope` helper, deny-all Rego left to the slice-4 dispatcher (ADR-0038); `TrustBoundary` inference limited to capability + tenant, boundary tests as authz replays of evidence (ADR-0039).
